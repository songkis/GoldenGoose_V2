...
from config.comm_settings import AIG_ID, AIG_NM, acf_DIR
...
import UI.AIGoose_ui

Ui_AIGoose = UI.AIGoose_ui.Ui_Dialog  # 직접 참조


class CUIAIGoose(QDialog, Ui_AIGoose):
    def __init__(self, parent=None):
        super(__class__, self).__init__(parent)
        # self.setAttribute(Qt.WA_DeleteOnClose)
        Ui_AIGoose.__init__(self)
        self.setupUi(self)
        self.parent = parent
...
class AIGoose(BaseGoose):
    def instance(self):
        # UUID = uuid.uuid4().hex
        UUID = AIG_ID
        return AIGoose(Name=GG_NM, UUID=UUID)
    ...
    def __init__(self, Name, UUID):
        ...
        self.acf파일 = ""
        self.일괄매도시각 = "15:18:30"
        self.일괄매도수익율 = "-3.0~5.0"
        self.매수거래시간STR = "09:04:25-15:20:00"
        self.매도거래시간STR = "09:01:25-15:20:00"
        self.clock = None
        self.전량매도 = False
        ...
        logger.info(f"__init__ self.금일매도종목 {self.금일매도종목} ")

    #  BaseGoose의 parent 결함을 우회하기 위한 안전한 ZMQ 주문 집행 래퍼
    def modal(self, parent):
        ui = CUIAIGoose(parent=parent)
        ui.setModal(True)
    ...
    def OnReceiveMessage(self, systemError, messageCode, message):
        ...
        try:
            ...
            # 에러 코드 분류 및 피드백 (01425: 잔고부족, 01478: 매도가능수량 부족 등)
            ...
            if (
                ...
                if hasattr(self, "smart_executor") and self.smart_executor.active_tasks:
                    # 가장 최근에 등록되거나 실행 중인 종목 추출 (보통 1종목씩 순차 처리되므로 첫 번째 키 활용)
                    active_tickers = list(self.smart_executor.active_tasks.keys())
                    if active_tickers:
                        ticker = active_tickers[0]

                logger.error(
                    f"❌ [API Reject] {ticker} | Code: {msg_code_stripped} | Msg: {msg_text}"
                )
                ...
                # 64-bit 엔진에 즉시 실패 보고 (종목 락 해제 유도)
                ...
                logger.warning(
                    f"⚠️ [API Comm-Warning] Code: {msg_code_stripped} | Msg: {msg_text} (통신 에러 - retry logic 대기)"
                )

            elif messageCode == "00039" or messageCode == "00040":  # 매도, 매수완료
                self.parent.주문가능금액부족 = False
                logger.info(f"✅ [Order Confirmed] {msg_text}")
    ...
    # OnReceiveRealData, adve_realdata_worker 등 공통 함수는 BaseGoose 통합
        ...
        if dfAccInfo.empty or (
            ...
            for k, v in port.items():
                ...
                if 수익율_val <= min_profit or 수익율_val >= max_profit:
                    ...
                    self.batSellQ.put(v)
                    logger.info(
                        f"🚀 [Profit Threshold Exit-AI] {v.종목명}({v.종목코드}) 수익율 {수익율_val}% 가 범위({min_profit}~{max_profit})를 벗어나 일괄매도 큐에 추가함."
                    )
    ...
    def setup_event_handlers(self):
        ...
        self.XQ_t1857 = t1857(parent=self, 식별자=AIG_ID)  # uuid.uuid4().hex)
        self.XQ_t1857.Query(
    ...
    def prepareRun(self):
        ...
        print("코스피 매수대상 종목 수 : %s" % len(self.kospi_codes))
        print("코스닥 매수대상 종목 수 : %s" % len(self.kosdaq_codes))
    ...
    def Lay(self, flag=True, parent=None):
        ...
        if flag:
            ...
            # 뉴스종목코드 DB -> LIST
            # 검색시작일자 = (datetime.datetime.now() - datetime.timedelta(days=3)).strftime('%Y%m%d')
            # query = "SELECT DISTINCT 종목코드 FROM 뉴스종목 WHERE 날짜 >= '%s' ORDER BY 키값 DESC " % ( 검색시작일자)
            # 뉴스종목코드 = pdsql.read_sql_query(query, con=conn)
            # self.parent.newsStockCodes = 뉴스종목코드.query('종목코드 is not None')['종목코드'].values.tolist()
            ##뉴스등록 DB->LIST 테스트
            ...
            logger.info(
                f"self.parent.gooses[1].portfolio.keys() : {self.parent.gooses[1].portfolio.keys()}"
            )
            logger.info(
...
def goose_loader():
    # UUID = uuid.uuid4().hex
    UUID = AIG_ID
    goose = AIGoose(Name=GG_NM, UUID=UUID)
    return goose
